...
class Personaje:
    def __init__(self, nombre, clase, fuerza, destreza, constitucion, inteligencia, sabiduria, carisma):
        ...
        self.nivel = 0  # Nivel del personaje
    ...
    def subir_nivel(self):
        self.nivel += 1
        self.vida_maxima += 5 + self.modificador(self.constitucion)
        self.vida_actual = self.vida_maxima
        print(f"{self.nombre} ha subido al nivel {self.nivel}! ¡Vida máxima aumentada a {self.vida_maxima}!")

    def __str__(self):
        return f"{self.nombre} ({self.clase}) - Nivel: {self.nivel} - Vida: {self.vida_actual}/{self.vida_maxima}"
...
class DungeonMaster:
    def __init__(self):
        ...
        self.orden_turnos = []  # Orden de turnos precalculado
        self.turno_actual = 0  # Turno actual
    ...
    def determinar_orden_turnos(self):
        ...
        # Almacenamos las tiradas de iniciativa en un diccionario
        iniciativas = {}
        for participante in self.jugadores + self.enemigos:
            iniciativa = participante.iniciativa()
            iniciativas[participante] = iniciativa

        # Ordenamos a los participantes por iniciativa
        self.orden_turnos = sorted(
            iniciativas.keys(),
            key=lambda x: iniciativas[x],
            reverse=True
        )

        # Mostramos el orden final de los turnos
        print("\nOrden de los turnos:")
        for i, participante in enumerate(self.orden_turnos):
            print(f"{i + 1}: {participante.nombre}")

    def combate(self):
        print("¡Comienza el combate!")
        self.determinar_orden_turnos()  # Calculamos el orden de los turnos una sola vez

        while any(jugador.esta_vivo() for jugador in self.jugadores) and any(enemigo.esta_vivo() for enemigo in self.enemigos):
            # Iteramos sobre el orden precalculado
            for i in range(len(self.orden_turnos)):
                participante = self.orden_turnos[self.turno_actual % len(self.orden_turnos)]
                ...
                self.turno_actual += 1  # Avanzamos al siguiente turno
                print(participante)
                if participante in self.jugadores:
        ...
        if any(jugador.esta_vivo() for jugador in self.jugadores):
            ...
            for jugador in self.jugadores:
                jugador.subir_nivel()  # Los jugadores suben de nivel después de ganar el combate
            self.agregar_mas_enemigos()  # El DM puede agregar más enemigos
        else:
            print("¡Los enemigos han ganado el combate!")

    def agregar_mas_enemigos(self):
        while True:
            opcion = input("DM, ¿quieres agregar más enemigos? (s/n): ").lower()
            if opcion == "s":
                num_enemigos = int(input("¿Cuántos enemigos quieres agregar? "))
                for i in range(num_enemigos):
                    nombre = f"Enemigo {len(self.enemigos) + 1}"
                    ...
                    print(f"Se ha agregado un nuevo enemigo: {enemigo.nombre}")
                break
            elif opcion == "n":
                break
            else:
